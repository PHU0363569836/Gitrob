...
func GetUserOrOrganization(login string, client *github.Client) (*GithubOwner, error) {
  ...
  return &GithubOwner{
    Login:     user.Login,
    ID:        intPointerFromInt64(user.ID),
    Type:      user.Type,
    Name:      user.Name,
...
func intPointerFromInt64(i *int64) *int {
    if i == nil {
        return nil
    }
    intValue := int(*i)
    ...
    for _, repo := range repos {
      if !*repo.Fork {
        r := GithubRepository{
          ...
          ID:            intPointerFromInt64(repo.ID),
          Name:          repo.Name,
...
func GetOrganizationMembers(login *string, client *github.Client) ([]*GithubOwner, error) {
  ...
  for {
    ...
    for _, member := range members {
      allMembers = append(allMembers, &GithubOwner{Login: member.Login, ID: intPointerFromInt64(member.ID), Type: member.Type})
    }
    if resp.NextPage == 0 {
      break
    }
